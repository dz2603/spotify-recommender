...
def page_dim_reduction():
    ...
    if st.button("▶ Run Dimensionality Reduction", type="primary"):
        ...
        # ── Autoencoder ───────────────────────────────────────────────────
        from src.algorithms.autoencoder import fit_autoencoder, TORCH_AVAILABLE
        ...
        st.info(
            ...
            "**3. Note on the K=5 Baseline:** The colors in the scatter plots above are generated by running K-Means with a fixed K=5 in the original high-dimensional space — used purely as a visual reference frame. The actual comparative analysis of different K values and clustering algorithms (K-Means vs. GMM) is available in the **Clustering** page, where K is fully user-adjustable."
        )
...
def page_recommendation_evaluation():
    ...
    # Load full dataset and numeric feature matrix
    data, X_num, feat_cols, _ = load_data(sample_n=None)
    ...
    if len(idxs) > 1:
        idxs_sorted = sorted(
            idxs[:20],
            key=lambda i: data.iloc[i]["popularity"],
            reverse=True,
        )
        opts = [
            f"{data.iloc[i]['track_name']} — {data.iloc[i]['artists']}"
    ...
    st.success(
        f"🎧 Reference: **{ref_row['track_name']}** by *{artists_str}*"
    )
    ...
    # For diversity_score, use standardized numeric features from X_num.
    # This avoids raw-scale features like tempo dominating the distance.
    ...
    m1.metric(
        "Same Genre Rate",
        "N/A" if same_genre is None else f"{same_genre:.0%}",
    )

    m2.metric(
        "Same Artist Rate",
        "N/A" if same_artist is None else f"{same_artist:.0%}",
    )

    m3.metric(
        "Avg Popularity",
        "N/A" if avg_pop is None else f"{avg_pop:.2f}",
    )

    m4.metric(
        "Popularity Gap",
        "N/A" if pop_gap is None else f"{pop_gap:.2f}",
    )

    m5.metric(
        "Diversity Score",
        "N/A" if diversity is None else f"{diversity:.4f}",
    )
